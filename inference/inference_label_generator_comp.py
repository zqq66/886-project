...
class Inference_label_comp(object):
    ...
    def __next__(self):
        ...
        while len(finishing_seq) < batch_size:
            ...
            with torch.no_grad():
                next_dist = self.model.tgt_get(src=mem,
                                               tgt=tgt,
    ...
    def obtain_glycan_mass(self, next_tgts, precursor_mass):
        ...
        for i,tgt in enumerate(next_tgts):
            ...
            mass_list = torch.tensor(
                [self.detokenize_aa_dict[i.item()] for i in tgt])
            ...
            glycan_crossattn_item = torch.stack([nmass, mass_list], dim=-1)
